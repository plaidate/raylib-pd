...
int main(void)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        // Playdate: move box B with the d-pad
        if (IsKeyDown(KEY_RIGHT)) boxB.x += 4;
        if (IsKeyDown(KEY_LEFT)) boxB.x -= 4;
        if (IsKeyDown(KEY_DOWN)) boxB.y += 4;
        if (IsKeyDown(KEY_UP)) boxB.y -= 4;
