...
int main(void)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        // Update
        //----------------------------------------------------------------------------------
        if (IsKeyPressed(KEY_ENTER)) boundedT = !boundedT;   // Playdate: B toggles bounded time
        ...
        // Playdate: crank drives d (duration)
        float dCrank = GetCrankChange()*0.02f;
        bool crankMoved = (fabsf(dCrank) > 0.001f);
        d += dCrank;
        if (d < D_MIN) d = D_MIN;
        if (d > D_MAX) d = D_MAX;
