...
int main(void)
{
    ...
    // Playdate: pre-spawn a population so the benchmark shows something headless
    for (int i = 0; i < 500; i++)
    {
        bunnies[bunniesCount].position = (Vector2){ (float)GetRandomValue(20, 380), (float)GetRandomValue(20, 220) };
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        // Update
        //----------------------------------------------------------------------------------
        if (IsKeyDown(KEY_SPACE))   // Playdate: hold A to spawn bunnies
        {
            // Create more bunnies
            for (int i = 0; i < 100; i++)
            {
