...
int main(void)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        // Playdate: d-pad up/down walks, left/right or crank turns
        float mazeYaw = GetCrankChange()*0.5f + (IsKeyDown(KEY_RIGHT)? 2.0f : 0.0f) - (IsKeyDown(KEY_LEFT)? 2.0f : 0.0f);
        float mazeFwd = (IsKeyDown(KEY_UP)? 0.1f : 0.0f) - (IsKeyDown(KEY_DOWN)? 0.1f : 0.0f);
        UpdateCameraPro(&camera, (Vector3){ mazeFwd, 0.0f, 0.0f }, (Vector3){ mazeYaw, 0.0f, 0.0f }, 0.0f);
