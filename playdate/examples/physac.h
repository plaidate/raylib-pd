...
static void InitTimer(void)
{
    srand(time(NULL));              // Initialize random seed
        ...
        // raylib-pd provides the platform clock through GetTime() (simulator:
        // CLOCK_MONOTONIC; device: pd->system->getElapsedTime()), in seconds
        frequency = 1000000000;     // GetTime() seconds -> nanosecond ticks
    #elif defined(_WIN32)
...
static uint64_t GetTimeCount(void)
{
    ...
    #if defined(PLATFORM_PLAYDATE)
        value = (uint64_t)(GetTime()*1.0e9);    // raylib-pd platform clock
    #elif defined(_WIN32)
